...
// Fungsi untuk mengecek apakah seorang pengguna memenuhi kriteria
function isUserEligible(user) {
  const hasGraceAsFriend = user.friends.some(
    (friend) => friend.name === "Grace"
  );
  const hasAppleOrBananaAsFavoriteFruit = ["apple", "banana"].includes(
    user.favoriteFruit
  );

  return hasGraceAsFriend && hasAppleOrBananaAsFavoriteFruit;
}
