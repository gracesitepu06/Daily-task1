...
// Fungsi untuk menentukan apakah tanggal registered sesudah tahun 2018
function isRegisteredAfter2018(dateString) {
  const registeredDate = new Date(dateString);
  return registeredDate.getFullYear() > 2018;
}

// Fungsi untuk menghitung jumlah pengguna yang memenuhi kriteria
function countUsersBelow30AndRegisteredAfter2018(users) {
  let count = 0;

  for (const user of users) {
    const userAge = user.age;
    const userRegisteredDate = user.registered;

    // Periksa apakah umur kurang dari 30 dan terdaftar sesudah tahun 2018
    if (userAge < 30 && isRegisteredAfter2018(userRegisteredDate)) {
      count++;
    }
  }
...
const jumlahUsers = countUsersBelow30AndRegisteredAfter2018(users);
console.log(
  `Jumlah pengguna yang berumur di bawah 30 dan terdaftar setelah tahun 2018 adalah: ${jumlahUsers}`
);
