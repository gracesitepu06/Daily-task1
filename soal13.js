...
// Fungsi untuk menukar nama depan dan belakang pada setiap pengguna
function swapFirstNameAndLastName(user) {
  const nameParts = user.name.split(" "); // Membagi nama menjadi potongan kata
  if (nameParts.length === 2) {
    // Pastikan ada tepat dua potongan kata (nama depan dan nama belakang)
    const [firstName, lastName] = nameParts;
    // Menukar nama depan dan belakang
    user.name = `${lastName} ${firstName}`;
  }
  return user;
}

// Mengaplikasikan fungsi swapFirstNameAndLastName ke setiap pengguna
const usersWithSwappedNames = users.map(swapFirstNameAndLastName);

console.log("Pengguna dengan nama yang ditukar:");
console.log(usersWithSwappedNames);
