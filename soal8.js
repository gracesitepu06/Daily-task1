...
const findCompanyName = (letter) => {
  ...
  for (let i = 0; i < users.usersData.length; i++) {
    const company = users.usersData[i].company;
  ...
  for (let i = 0; i < splitCompanyNameData.length; i++) {
    if (splitCompanyNameData[i][0] === letter) {
      userCompanyNameDataFilter.push(users.usersData[i].name);
    }
  }

  console.log(userCompanyNameDataFilter);

  return `User yang company huruf depannya berawal ${letter} yaitu sebanyak ${userCompanyNameDataFilter.length} user`;
};
