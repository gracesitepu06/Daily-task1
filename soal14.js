const users = [
  {
    ...
    address:
      "435 Livonia Avenue, Keyport, Federated States Of Micronesia, 6768",
...
function changeString(users) {
  // melakukan perulangan / looping dalam users
  for (let i = 0; i < users.length; i++) {
    // users index i dengan key company diubah menjadi binar
    users[i].company = "binar";
    // menyimpan users index i key email ke variabel email
    let email = users[i].email;
    // mengambil email index ke 0 sampai @, kemudian disimpan di variabel output
    let output = email.substring(0, email.lastIndexOf("@"));
    // mengubah users index ke i dengan key email menjadi isi dari variabel output kemudian ditambahkan '@binar.org'
    users[i].email = output + "@binar.org";
  }
  // lalu mengembalikan nilai users yang sudah diubah
  return users;
}

// memanggil function
console.log(changeString(users));
