...
function isUserEligible(user) {
  const isNameShort = user.name.length < 10;
  const isAgeAbove30 = user.age > 30;
  const isMaleWithBrownEyes =
    user.gender === "male" && user.eyeColor === "brown";

  return isNameShort || isAgeAbove30 || isMaleWithBrownEyes;
}

// Filter pengguna yang memenuhi kriteria
const eligibleUsers = users.filter(isUserEligible);

// Tampilkan hasil
console.log("Pengguna yang memenuhi kriteria:");
console.log(eligibleUsers);

console.log(
  `Jumlah pengguna yang memenuhi kriteria adalah: ${eligibleUsers.length}`
);
